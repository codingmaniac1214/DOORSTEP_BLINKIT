...
const SuggsCNN = (props) => {
  ...
  const MLBackend = async () => {
    ...
    let itemurls = [];
    itemdetail.map((data) => {
      itemnames.push(data.product_name);
      itemurls.push(data.image);
    });

    const res = await axios.post("http://127.0.0.1:5000/cnnArch", {
      data: itemnames,
      urls: itemurls,
    });
    // console.log("this is the response from python");
    // console.log(res);
    let names = res["data"].data;
    console.log(names.data);

    const search_res = async (words) => {
      // words.map((str) => {
      axios.post(url + "/api/searchproducts", { str: words }).then((res) => {
        if (res.data.length > 0)
          setFinal_Items((prev) => [...prev, res.data[0]]);
      });
      // });
    };

    const search_handler = (search_str) => {
      setFinal_Items([]);
      let words = search_str.toLowerCase();

      // for (let i = 0; i < words.length; i++) {
      // }
      words = words[0].toUpperCase() + words.slice(1);

      setFinal_Items((n) => n.splice(0, n.length));
